...
public partial class MegamanX : Character {
	...
	public void shoot(int chargeLevel) {
		...
		if (chargeLevel >= 2 && player.weapons.Any(w => w is HyperCharge c)) {
			var hcWep = player.weapons.FirstOrDefault(w => w is HyperCharge) as HyperCharge;
			if (hcWep != null) {
		...
		if (chargeLevel >= 3 && player.hasArmArmor(2) && player.weapon is not XBuster) {
			if (stockedX2Charge) {
				shootCooldown = 0;
		...
		if (!updatedStock && player.weapon is not XBuster) stockX2Charge(false);
	...
	public bool shouldShowHyperCharge() {
		return player.weapon is HyperCharge hc && hc.canShootIncludeCooldown(player) || flag != null;
	}
